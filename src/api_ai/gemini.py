...
- 반드시 실제 KRX(한국거래소)에 상장된 개별 기업만 추천하세요. 6자리 숫자 종목코드를 반드시 포함해야 합니다.
- ETF, 인버스, 펀드, 지수, 테마주, 섹터 같은 추상적 카테고리는 절대 추천하지 마세요. 반드시 구체적인 법인명과 종목코드가 있는 상장 기업이어야 합니다.
- 종목코드를 모르면 그 종목은 제외하고 코드를 아는 다른 종목으로 대체하세요.
...
6. 뉴스 내용을 바탕으로 KOSPI와 KOSDAQ 시장에 미칠 영향을 헤드라인 형식으로 각 3개씩 작성합니다. 뉴스에 명시적인 시장 헤드라인이 없더라도 종목/산업 뉴스에서 유추하여 반드시 작성하세요.
